...
7. **SEO + social-card wiring** — every new lab needs full meta
   tags, three JSON-LD blocks, a per-lab OG image, a per-lab
   Twitter thumbnail, a sitemap entry, an llms.txt entry, and a
   Related-labs aside. See `references/seo.md` for the complete
   contract; without these, the lab is effectively invisible to
   Google, Bing, and AI search, and link previews on Twitter or
   LinkedIn render with a generic icon instead of a card.

   Run the helpers in this order (all four are required, not just
   the OG ones — Twitter scrapes a different URL):
   - `python3 notebooks/generate_og_images.py` (per-lab OG, 1200×630)
   - `python3 notebooks/wire_og_images.py` (rewrites og:image lines)
   - `python3 notebooks/generate_tweet_thumbnails.py` (per-lab
     Twitter card, 1200×675, topic-focused; no brand wordmark)
   - `python3 notebooks/inject_twitter_cards.py` (rewrites
     twitter:image to point at `assets/twitter-thumbs/<slug>.png`
     on the **www** host)
   ...
   **Common failure mode (seen repeatedly):** `assets/twitter-thumbs/`
   is git-untracked at first creation. The thumbnails exist
   locally and the meta tags point at them, but the directory
   never makes it into the commit, so Twitter's scraper gets a
   404 and shows the generic newspaper-icon placeholder. Always
   `git add assets/twitter-thumbs/` after running the scripts.
...
9. **Run the QA gate**: `python3 .claude/skills/portfolio-lab-creator/scripts/verify_lab.py <slug>`.
   ...
   - SEO contract: per-lab OG image AND per-lab Twitter thumbnail
     both exist at the expected paths, both image URLs point at
     `https://www.jakecuth.com/...` (not bare apex),
     `assets/twitter-thumbs/` is tracked in git, `sitemap.xml`
     has a `<url>` entry for the new slug, `llms.txt` mentions
     the new slug, canonical and og:url present, and the
...
- [ ] **SEO contract from `references/seo.md`:**
  ...
  - [ ] `assets/twitter-thumbs/<slug>-lab.png` exists (1200×675) —
        generated by `notebooks/generate_tweet_thumbnails.py`
  - [ ] `assets/twitter-thumbs/` is tracked in git (it has been
        created untracked multiple times — `git add` it explicitly)
  - [ ] `og:image` and `twitter:image` URLs use
        `https://www.jakecuth.com/...` (not bare apex)
  - [ ] `twitter:image` points at `/assets/twitter-thumbs/` (topic
        thumbnail), NOT `/assets/og/` (branded card)
